...
# Base directory: the folder containing the exe when running as a packaged
# build, or the folder containing this script when running from source.
# Using sys.executable's folder (instead of __file__) is required for a
# PyInstaller --onefile build, since __file__ there points into a temporary
# extraction directory, not the folder the user actually placed the exe in.
...
def get_helper_cmd(logical_name, in_tests_subdir=False):
    """Build the subprocess argv for a helper script/companion exe.

    In a packaged build, each helper (FirmwareFlash.py, scripts/tests/*.py)
    is compiled into its own companion exe that sits next to the main exe
    (test scripts under a "tests" subfolder). In source form, they're run
    with the current Python interpreter instead.
    """
    if getattr(sys, "frozen", False):
        exe_dir = os.path.join(BASE_DIR, "tests") if in_tests_subdir else BASE_DIR
        return [os.path.join(exe_dir, f"{logical_name}.exe")]
    script_dir = os.path.join(BASE_DIR, "scripts", "tests") if in_tests_subdir else BASE_DIR
    return [sys.executable, os.path.join(script_dir, f"{logical_name}.py")]
...
def insert_into_ems_text(data_row):
    if data_row[2] == "":
        ...
        with open(result_file_path, mode, encoding="utf-8") as f:
            ...
            f.write(f"Digital Input Result :- {data_row[19]}\n")
            f.write(f"Digital Input Value :- {data_row[20]}\n")
            f.write(f"MD Reset Button Result :- {data_row[21]}\n")
            f.write(f"MD Reset Button Value :- {data_row[22]}\n")
            f.write(f"Magnet Status Result :- {data_row[23]}\n")
            f.write(f"Magnet Status Value :- {data_row[24]}\n")
            f.write(f"Push Button 1 Result :- {data_row[25]}\n")
            f.write(f"Push Button 1 Value :- {data_row[26]}\n")
            f.write(f"Push Button 2 Result :- {data_row[27]}\n")
            f.write(f"Push Button 2 Value :- {data_row[28]}\n")
            f.write(f"RX/TX Status Result :- {data_row[29]}\n")
            f.write(f"RX/TX Status Value :- {data_row[30]}\n")
            f.write(f"eFuse Mode Result :- {data_row[31]}\n")
            f.write(f"eFuse Mode Value :- {data_row[32]}\n")
            f.write(f"Cover Open Result :- {data_row[33]}\n")
            f.write(f"Cover Open Value :- {data_row[34]}\n")
...
class ExcelLogger:
    ...
    def __init__(self, main_file, detailed_file):
        ...
        # In-memory workbooks — loaded once, saved only at end of each cycle
        self._wb_main = None
        self._wb_detailed = None
    ...
    def initialize_excel_files(self):
        ...
        with self.lock:
            ...
            # Reset caches so _get_wb_* load the freshly created files
            self._wb_main = None
            self._wb_detailed = None

    def _get_wb_detailed(self):
        """Return the in-memory detailed workbook, loading it if needed."""
        if self._wb_detailed is None:
            self._wb_detailed = load_workbook(self.detailed_file)
        return self._wb_detailed

    def _get_wb_main(self):
        """Return the in-memory main workbook, loading it if needed."""
        if self._wb_main is None:
            self._wb_main = load_workbook(self.main_file)
        return self._wb_main
    ...
    def log_test_start(self, session_id, station, serial_number):
        """Log when a test session starts (append only; save deferred to log_final_results)."""
        with self.lock:
            try:
                ws = self._get_wb_detailed().active
    ...
    def log_individual_test(self, session_id, station, serial_number, test_name,
        ...
        """Log individual test details (append only; save deferred to log_final_results)."""
        with self.lock:
            try:
                ws = self._get_wb_detailed().active
    ...
    def log_final_results(self, session_id, station, serial_number, test_results,
        ...
        """Log final test results and flush both workbooks to disk (once per cycle)."""
        with self.lock:
            try:
                wb = self._get_wb_main()
                ws = wb.active
                ...
                # Flush both workbooks to disk once per cycle
                wb.save(self.main_file)
                if self._wb_detailed is not None:
                    self._wb_detailed.save(self.detailed_file)
...
class ManufacturingSuite:
    def __init__(self, root):
        self.root = root
        self.root.title("Smart Manufacturing Suite : 3P- [SMS] [Version 1.0.0.2, 09-July-26]")
        ...
        self.scanner_ports = {}
        self.optical_ports = {}
        self.load_com_port_config(os.path.join(BASE_DIR, "config.txt"))

        # Load firmware flash configuration (gang .cfg file + which gang
        # programming position each station is wired to)
        self.gang_config_file = "3Ph_GangConfig.cfg"
        self.station_gang_map = {1: "#1", 2: "#2"}
        self.load_firmwareflash_config()
    ...
    def load_firmwareflash_config(self):
        """Load which gang .cfg file to flash with and which gang programmer
        position each station is wired to from firmwareflash_config.json.
        Falls back to the previous hardcoded defaults if the file is
        missing or invalid, so a bad/missing config never blocks the line."""
        config_path = os.path.join(BASE_DIR, "firmwareflash_config.json")
        try:
            if os.path.exists(config_path):
                ...
                if codefile:
                    self.gang_config_file = codefile

                station_1_map = data.get("station_1_gang_pro_map")
                if station_1_map:
                    self.station_gang_map[1] = station_1_map

                station_2_map = data.get("station_2_gang_pro_map")
                if station_2_map:
                    self.station_gang_map[2] = station_2_map

                logging.info(f"Loaded firmwareflash_config.json: codefile={self.gang_config_file}, "
                             f"station_gang_map={self.station_gang_map}")
            else:
                logging.warning(f"firmwareflash_config.json not found at {config_path}; "
                                 f"using defaults codefile={self.gang_config_file}, "
                                 f"station_gang_map={self.station_gang_map}")
        except Exception as e:
            logging.error(f"Failed to load firmwareflash_config.json: {e}; using defaults")
    ...
    def create_header(self):
        ...
        # Add version info
        version_label = tk.Label(header_frame, text="v1.0.0.2 | 09-July-26", 
    ...
    def get_firmware_version_from_file(self):
        ...
        try:
            firmware_config = os.path.join(BASE_DIR, "ConfigFiles", "firmware_config.txt")
    ...
    def _run_efuse_mode_test(self, com_port, serial_number, station_num, task_name):
        ...
        try:
            ...
            set_cmd = get_helper_cmd("set_efuse_mode", in_tests_subdir=True) + ["--serial_port", com_port, "--enable", str(random_efuse_value)]
            ...
            if set_success:
                ...
                get_cmd = get_helper_cmd("get_efuse_mode", in_tests_subdir=True) + ["--serial_port", com_port]
    ...
    def _run_test_script(self, script_name, com_port, serial_number, station_num, task_name, result_key, value_key=None):
        ...
        logical_name = script_name[:-3] if script_name.endswith(".py") else script_name
        cmd = get_helper_cmd(logical_name, in_tests_subdir=True) + ["--serial_port", com_port]
            ...
            if task_name == "Memory Test":
                ...
                if "Digital Input Status:" in raw_output:
                    ...
                    if input_match:
                        ...
                        test_status = "PASS"  # Any digital input status reading is PASS
                    else:
                        test_status = "FAIL"
                        test_value = "N/A"
                else:
    ...
    def FirmwareFlash(self, is_main_firmware=False):
        ...
        flash_logical_name = "MainFirmwareFlash" if is_main_firmware else "FirmwareFlash"
        ...
        folder_path = os.path.join(BASE_DIR, 'ConfigFiles')
        Config_file = self.gang_config_file
        cmd = get_helper_cmd(flash_logical_name) + [os.path.join(folder_path, Config_file)]
        ...
        try:
            ...
            for station_num, serial_number in active_stations_for_flash:
                ...
                with self.test_completion_lock:
                    ...
                    gang_position = self.station_gang_map.get(station_num)
                    station_flash_ok = flash_success and gang_position and result_station_dict.get(gang_position) == "OK"
    ...
    def _run_all_tests_sequence(self):
        ...
        # Digital Input Test
        self._execute_tests_in_parallel("Digital Input Test", "digital_input_test.py", "DIGITAL_INPUT_RESULT", "DIGITAL_INPUT_VALUE")
    ...
    def check_all_pass(self, data):
        if not data:
            return False
        if all((test.get("status") == "PASS" ) or (test.get("status") == "OK") for test in data.values()) and len(data)==len(self.completed_tests):
            return True
        else:
            return False
